...
void eventloop_once(reactor_t * r, int timeout) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        int mask = e->events;
        if (e->events & EPOLLERR) mask |= EPOLLIN | EPOLLOUT; // 具体错误由io函数去获取
...
int flush_socket(event_t *e) {
    ...
    if (dis >= len) {
        ...
        int remain = len - dis;
        if (remain > 0) {
            n = write_socket_(e, r->buf + REAL_READ_POS(r), remain);
            if (n == 0 || n < remain) {
                r->read_pos += n;
                return 0;
            } else if (n < 0)
...
int write_socket(event_t *e, void * buf, int sz) {
    ...
    if (rb_isempty(r)) {
        ...
        if (n == 0 || n < sz) {
            rb_write(&e->out, (char *)buf+n, sz-n);
